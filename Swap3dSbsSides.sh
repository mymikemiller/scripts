#!/bin/bash
#
# Developed by Mike Miller 2020-05-04, based on code from
# "mirror", developed by Fred Weinhaus 12/4/2014 .......... revised 4/25/2015
#
# ------------------------------------------------------------------------------
# 
# Licensing:
...
# \( -clone 0 -crop ${ccol}x+0+0 +repage \( +clone \) +repage \( +clone \)\
# 	+append +write "${outname}_left.$suffix" \) \

# process image
convert -respect-parenthesis $tmpA1 +repage \
\( -clone 0 -crop ${ccol}x+0+0${wr}x+${ccol}+0 +repage \( -clone 0 -crop ${ccol}x+0+0 \) \
	+append +write "${outname}_left.$suffix" \) \
